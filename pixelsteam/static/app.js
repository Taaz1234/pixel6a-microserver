/* =========================================================
   PixelSubs Pro - Logic & Interactivity v2.2
   With Official Pixel-Perfect Brand SVG Logos
   ========================================================= */
...
function getBrandLogo(sub, size = 36) {
  ...
  return `<img src="/icons/${iconId}.svg" width="${size}" height="${size}" alt="${sub.name}" class="brand-logo-svg">`;
}
  ...
  subsGrid.innerHTML = filtered.map(sub => {
    ...
    return `
      <div class="sub-card" onclick="openDetailModal('${sub.id}')">
        ...
        <div class="sub-header-banner" style="background: linear-gradient(135deg, ${sub.color}30, #111827);">
...
function openDetailModal(subId) {
  ...
  detailModalContent.innerHTML = `
    <div class="modal-header-hero">
      <div style="width: 85px; height: 85px; border-radius: var(--radius-md); background: ${sub.color}; display: flex; align-items: center; justify-content: center; box-shadow: var(--shadow-md); flex-shrink: 0; padding: 0.5rem;">
        ${brandSvg}
      </div>
